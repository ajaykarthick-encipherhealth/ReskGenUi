...
export async function individualUsersList({ data }) {
  ...
  const res = await requestPortal(
    `dbservice/auditor/patient/filter?page=${data?.pageNo}&size=15&userId=${data?.uId}&isAllocation=false&processedStatus=${filteredStatus || ""}&auditedStatus=${data?.selectedAuditOption}&searchString=${data?.search}&processedStart=${data?.completedStartDate}&processedEnd=${data?.completedEndDate}&auditedDateStart=${data?.aduitCompletedStartDate}&auditedDateEnd=${data?.aduitCompletedEndDate}&dueDateStart=${data?.dueStartDate}&dueDateEnd=${data?.dueEndDate}&auditDueDateStart=${data?.aduitDueStartDate}&auditDueDateEnd=${data?.aduitDueEndDate}&allocatedBy=${data?.selAllocatedBy}&auditAllocatedBy=${data?.selAuditAllocatedBy}&auditDueDateStart=${data?.auditedStartDate}&auditDueDateEnd=${data?.auditedEndDate}&allocatedOnStart=${data?.allocatedStartDate}&allocatedOnEnd=${data?.allocatedEndDate}&sortfield=${data?.sort?.sortField}&sortdirection=${data?.sort?.sortDir}&priority=${data?.priority||""}
     `,
    options
  );
  return res;
