...
export const getPatientLabDetailsNew = async (
  ...
  if (patientDetailsResult?.data?.response) {
    ...
    var dosYearArrFile = [];
    var fileDatesArr = [];
    if (result?.labFileDetail) {
      if (result.labFileDetail.length != 0) {
        result.labFileDetail.map((res, index) => {
          for (var key in res.documentDos) {
            fileDatesArr.push({ value: key, label: key });
          }
        });
    ...
    if (result.hccDiseases != null) {
      ...
      var newArrayColorMatchs = [];
      newArrayColorMatchs = [...notMatchColorArray, ...notMatchColorArrayMeat];
