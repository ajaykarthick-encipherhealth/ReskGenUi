...
import { filterWidgetsByRole, InvalidWidget, useHasMounted, useWindowWidth } from "../../function";
import { getColSpan } from "../../function";
import { getRowSpan } from "../../function";
import DndFunction from "../../function/resubaleDndContext";
import { connect } from "react-redux";
import { DOSCount, InvalidCredentails, InvalidDocument, MRNIDMismatch, MultiplePatientFound, PatientDeceased, PatientDOBMismatch, PatientInActive, PatientNameMismatch, ScopeYearMis, ScopeYearMismatch, Televisit } from "./mockData";
...
function Invalid({
  ...
  const getCharts = ({ type, chartType, chartChange }) => {
    switch (type) {
      case "DOSCount":
        return (
          <DOSCount chartType={chartType} chartChange={chartChange} />
        );
      ...
      case "Televisit":
        return (
          <Televisit chartType={chartType} chartChange={chartChange} />
        );
      ...
      case "PatientNameMismatch":
        return (
        <PatientNameMismatch chartType={chartType} chartChange={chartChange} />
        );
      ...
      case "MultiplePatientFound":
        return (
          <MultiplePatientFound chartType={chartType} chartChange={chartChange} />
        );
  ...
  return (
    <>
      {isDragable ? (
        ...
        >
          {filterWidgetsByRole(InvalidWidget, selectedRole)?.sort((a,b) => (a.orderValue - b.orderValue))?.map((item, id) => {
            const style = {
