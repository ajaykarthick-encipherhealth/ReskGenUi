...
export default function DndFunction({
  ...
  const fullDashboard = useMemo(
    () => generateGridWithPlaceholders(dashboard),
    [dashboard]
  );
  return (
    <DndContext
    ...
    >
      <SortableContext
        id="dashboard-zone"
        items={fullDashboard.map((i) => i.widgetId)}
      ...
      <DragOverlay dropAnimation={null}>
        {activeItem && (
          <OverlayRenderer item={activeItem} activeBtn={"Default"} />
        )}
      </DragOverlay>
    </DndContext>
  );
